...
void* testMode(void *id ) 
{
	...
	for (int ii = ll; ii < rr; ii++)
	{
		...
		int k = getRand(0,bags_test[b[ii]].size());
		int j = bags_test[b[ii]][k];

		vector<double> score = test(testtrainLists[j],  testPositionE1[j], testPositionE2[j], testtrainLength[j], r);
...
void test() {
...

	{
		...
		for (int i=0; i<min(20000,int(aa.size())); i++)
		{
			...
			f1_tmp = 2*(correct/(i+1))*(correct/tot)/(correct/(i+1)+correct/tot);
			if (f1<f1_tmp) f1 = f1_tmp;
		}
		cout<<"f1: "<<f1<<endl;
		fclose(f);
		if (!output_model)
